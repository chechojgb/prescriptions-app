...
export default function AdminPrescriptionsPage() {
  ...
  async function fetchPrescriptions() {
    ...
    try {
      ...
      const { data } = await api.get(`/admin/prescriptions?${params.toString()}`);
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-black text-[#371851] dark:text-white">Todas las Prescripciones</h1>
        <p className="text-sm text-[#371851]/50 dark:text-white/50 mt-1">{total} prescripciones en el sistema</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Buscar por médico, paciente o código..."
            value={search}
      ...
      {loading ? (
        ...
        <div className="space-y-3">
          {filtered.map((p) => (
            <div
              key={p.id}
              className="flex items-center gap-4 p-5 rounded-2xl border border-[#371851]/10 dark:border-white/10 bg-white dark:bg-white/5 hover:border-[#18B0C6]/40 transition"
            >
              {/* Icono estado */}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="font-black text-sm text-[#371851] dark:text-white truncate">
                    {p.patient?.user?.name || 'Sin paciente'}
                  </span>
                  <span className="text-[#371851]/30 dark:text-white/30 text-xs">→</span>
                  <span className="text-sm text-[#371851]/60 dark:text-white/60 truncate">
                    Dr. {p.doctor?.user?.name || 'Sin médico'}
                  </span>
              ...
              <Link
                ...
                className="w-9 h-9 rounded-xl border border-[#371851]/10 dark:border-white/10 flex items-center justify-center hover:bg-[#371851] hover:border-[#371851] hover:text-white dark:hover:bg-white/10 transition text-[#371851]/50 dark:text-white/50 shrink-0"
              >
                <Eye size={15} />
              </Link>
            </div>
